...
完整体系集成测试 v2
====================
测试无极→太极→两仪→四象→八卦→六十四卦→万物的完整闭环系统。
"""
...
def test_complete_system():
    ...
    print("\n📝 测试 13：六十四卦状态系统")
    try:
        from modules.bagua.gua64.system import Gua64System
        g = Gua64System()
        
        # 初始状态
        state = g.get_current_gua()
        assert state.gua.number == 1
        ...
        stats = g.get_stats()
        assert stats["total_cycles"] >= 1
        assert stats["current_gua"]["number"] == 1  # 回到乾卦
        
        print(f"   ✅ 64 卦定义完整，循环 {stats['total_cycles']} 次")
        print(f"   ✅ 最终回到：{stats['current_gua']['number']}. {stats['current_gua']['name']}")
        results["gua64"] = "PASS"
    except Exception as e:
        print(f"   ❌ GUA64: {e}")
        results["gua64"] = f"FAIL: {e}"
