/**
 * StarCoreTweak.xm v5.8 - 修复tap/swipe卡死TCP server
 * 
 * v5.8 修复清单：
 * 1. 🔥 tap/swipe/longPress/pressHome/keyPress/textInput→dispatch_async到主线程异步执行
 *    根因：simulateTap_old→simulateTouch→getTargetContextID→runOnMainThreadSync(dispatch_sync)
 *    在TCP handler线程调用dispatch_sync阻塞→无法写回响应→客户端卡死
 *    修复：dispatch_async到主线程，TCP handler立即返回{"success":true,"status":"executing"}
 * 2. 🔥 shell命令PATH修复：添加/var/jb/路径前缀（Dopamine rootless越狱环境）
 * 3. 🔥 shell命令/bin/sh→/var/jb/bin/sh（Dopamine rootless无/bin/sh）
 * 
 * v5.7 修复清单：
...
static bool loadFunctions() {
    ...
    success = true;
    NSLog(@"[StarCoreTweak] ✅ v5.8 函数加载成功");
    return true;
}
...
- (void)start {
    ...
    NSLog(@"[StarCoreTweak] TCP :6000 v5.8 (修复tap/swipe卡死TCP)");
}
- (void)acceptLoop {
...
- (NSDictionary *)processMessage:(NSString *)jsonStr {
    ...
    // ★ v5.8: tap - 异步执行触摸注入，避免阻塞TCP handler
    // 根因：simulateTap_old→simulateTouch→getTargetContextID→runOnMainThreadSync→dispatch_sync阻塞TCP线程
    // 修复：dispatch_async到主线程，TCP handler立即返回
        ...
        // ★ v5.8: 触摸操作dispatch到主线程异步执行
        // simulateTap_old内部runOnMainThreadSync在主线程上直接执行(不阻塞)
    ...
    // ★ v5.8: swipe - 异步执行触摸注入，避免阻塞TCP handler
    ...
    // ★ v5.8: longPress - 异步执行触摸注入，避免阻塞TCP handler
    ...
    // ★ v5.8: keyPress - 异步执行，避免阻塞TCP handler
    ...
    // ★ v5.8: textInput - 异步执行，避免阻塞TCP handler
    ...
    // ★ v5.4: shell - 使用posix_spawn执行命令（绕过SpringBoard沙盒限制）
        ...
        if (!cmd || cmd.length == 0) {
            ...
            // 使用/var/jb/bin/sh -c执行（Dopamine rootless无/bin/sh），带完整PATH和root权限
            char *argv[] = {(char*)"/var/jb/bin/sh", (char*)"-c", (char*)[cmd UTF8String], NULL};
            char *envp[] = {
                (char*)"PATH=/var/jb/usr/bin:/var/jb/bin:/var/jb/usr/sbin:/var/jb/sbin:/var/jb/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin",
            ...
            pid_t pid;
            int spawnResult = posix_spawn(&pid, "/var/jb/bin/sh", &actions, NULL, argv, envp);
...
// ==================== SpringBoard Hook (手动swizzling，无需Logos) ====================

// MSHookMessageEx声明（ElleKit兼容CydiaSubstrate API）
extern "C" void MSHookMessageEx(Class _class, SEL message, IMP hook, IMP *old);

// 原始函数指针
static void (*_orig_SpringBoard_applicationDidFinishLaunching)(id, SEL, id);

// 替换函数
static void _replaced_SpringBoard_applicationDidFinishLaunching(id self, SEL _cmd, id application) {
    _orig_SpringBoard_applicationDidFinishLaunching(self, _cmd, application);
    NSLog(@"[StarCoreTweak] SpringBoard启动 v5.8 (修复tap/swipe卡死TCP)");
    loadFunctions();
    _server = [[StarCoreTCPServer alloc] init];
    [_server start];
    NSLog(@"[StarCoreTweak] v5.8: 虚拟设备需手动调用initDevice, shell命令已就绪(PATH含/var/jb/)");
}

// 构造函数（替代%ctor）
__attribute__((constructor)) static void _logosLocalInit() {
    NSLog(@"[StarCoreTweak] v5.8 loading... (修复tap/swipe卡死TCP)");
    
    // Hook SpringBoard
    Class sbClass = objc_getClass("SpringBoard");
    if (sbClass) {
        MSHookMessageEx(sbClass, @selector(applicationDidFinishLaunching:),
            (IMP)_replaced_SpringBoard_applicationDidFinishLaunching,
            (IMP *)&_orig_SpringBoard_applicationDidFinishLaunching);
    }
}

// 析构函数（替代%dtor）
__attribute__((destructor)) static void _logosLocalFini() {
    [_server stop];
}
